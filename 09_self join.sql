...
--4. The query shown gives the number of routes that visit either London Road (149) or Craiglockhart (53). Run the query and notice the two services 
...
WHERE stopa.name=(SELECT name FROM stops WHERE name = 'Craiglockhart')
AND stopb.name=(SELECT name FROM stops WHERE name = 'London Road')
